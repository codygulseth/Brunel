...
- `schedule_intelligence` owns immutable schedule revisions, source-preserving CSV/XML/XER normalization, schedule quality, conservative CPM evidence, activity lineage, deterministic comparison, workflow links, synchronization proposals, dashboards, API/CLI, audit, and cited Q&A. It has no write-back authority.
- `p6_adapter` owns Primavera transport capability declarations, safe XER/P6 XML parsing, project discovery, explicit external mapping, P6 orchestration, and P6-specific export validation. It delegates admission, identity, audit, approval, execution, reconciliation, comparison, quality, and retrieval to canonical services.
- `field_intelligence` owns immutable daily-report revisions, typed field observations, human review and internal issue, schedule-link/progress proposals, revision comparison, weekly summaries, dashboards, audit, API/CLI, and cited Q&A. Reported conditions never become contractual or schedule facts automatically.
